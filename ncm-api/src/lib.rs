...
impl MusicApi {
    ...
    /// track_ids: 歌曲 id 列表
    #[allow(unused)]
    pub async fn playlist_tracks(&self, op: &str, pid: u64, track_ids: &[u64]) -> bool {
        let path = "/weapi/playlist/manipulate/tracks";
        let mut params = HashMap::new();
        params.insert("op", op);
        let pid = pid.to_string();
        params.insert("pid", &pid);
        let track_ids_str = format!(
            "[{}]",
            track_ids
                .iter()
                .map(|id| id.to_string())
        ...
        params.insert("trackIds", &track_ids_str);
        if let Ok(result) = self
            .request(Method::Post, path, params, CryptoApi::Weapi, "", true)
            .await
        {
    ...
    /// 返回新歌单 ID
    #[allow(unused)]
    pub async fn playlist_create(&self, name: &str) -> Result<u64> {
        let path = "/weapi/playlist/create";
        let mut params = HashMap::new();
        params.insert("name", name);
        params.insert("privacy", "0");
        let result = self
            .request(Method::Post, path, params, CryptoApi::Weapi, "", true)
            .await?;
        to_playlist_create_id(result)
    }
