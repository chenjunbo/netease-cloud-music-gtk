            ...
            Parse::Radio => {
                ...
                for v in array.iter() {
                    vec.push(SongList {
                        ...
                        creator_id: get_val!(v, "dj", "userId").unwrap_or(0),
                    });
                }
            }
            _ => {}
...
/// 从创建歌单响应中提取新歌单 ID
#[allow(unused)]
pub fn to_playlist_create_id(json: String) -> Result<u64> {
    let value = &serde_json::from_str::<Value>(&json)?;
    let code: i32 = get_val!(value, "code")?;
    if code == 200 {
        // 尝试从 playlist.id 获取
        let id: Result<u64> = get_val!(@as u64, value, "playlist", "id");
        if let core::result::Result::Ok(id) = id {
            return Ok(id);
        }
        // 尝试从顶层 id 获取
        let id: Result<u64> = get_val!(@as u64, value, "id");
        if let core::result::Result::Ok(id) = id {
            return Ok(id);
        }
        return Err(anyhow!("playlist id not found in response"));
    }
    let msg: String = get_val!(value, "msg").unwrap_or_default();
    Err(anyhow!("create playlist failed: code={}, msg={}", code, msg))
}
